...
function Header() {
  ...
  return (
    <header className="header">
      <h1 style={style}>Fast React Pizza Food Co.</h1>
    </header>
  );
}
...
function Footer() {
  ...
  const openHour = 12;
  const closedHour = 17;
  const isOpen = hour >= openHour && hour <= closedHour; // Adjusted condition
  console.log(isOpen);
  return (
...
function Order({ closedHour, openHour }) {
  <div className="order">
    <p>
      We're open from {closedHour}:00 to {openHour}:00. Come visit us or order
      online.
    </p>
